...
//   val mktng_start_dt = args(3)
   ...
   //val correlation_path = s"hdfs://apollo-phx-nn-ha/user/hive/warehouse/mktng.db/correlation_parquet/ck_dt=$cko_dt/data_source=$data_source/mktng_dt=$mktng_dt"
   //println(s"the final destination is $correlation_path")

   val txns_path =  s"hdfs://apollo-phx-nn-ha/user/hive/warehouse/txns.db/txns_with_xid_parquet"
   ...
   val mktng_df  = spark.read.load(mktng_path).filter(s"dt between date_sub('$cko_start_dt', 40) and '$mktng_end_dt'")
   mktng_df.createOrReplaceTempView("mktng")


   //println(mktng_df.columns)
   //println("going to do some calculations")
   //val txns_count = txns_df.count()
   //val mktng_count = mktng_df.count()
   //println(s"there are $txns_count events in transactions and $mktng_count events in marketing")
   ...
   coalesce(A.device_level1, B.device_level1) as device_level1,
   coalesce(A.device_level2, B.device_level2) as device_level2,
   ...
   coalesce(A.ck_dt, B.ck_dt) as ck_dt
   ...
   coalesce(A.experience_level1, B.experience_level1) as experience_level1,
   coalesce(A.experience_level2, B.experience_level2) as experience_level2,
   coalesce(A.device_level1, B.device_level1) as device_type_level1,
   coalesce(A.device_level2, B.device_level2) as device_type_level2,
   coalesce(A.flex_column_1, B.flex_column_1) as flex_column_1,
   coalesce(A.flex_column_2, B.flex_column_2) as flex_column_2,
   coalesce(A.flex_column_3, B.flex_column_3) as flex_column_3,
   coalesce(A.flex_column_4, B.flex_column_4) as flex_column_4,
   ...
   //val merge_all_join_cnt = merge_all_join_df.count()
   //println(s"the merge has $merge_all_join_cnt values")
   //merge_all_join_df.repartition($"data_source", $"dt")
   merge_all_join_df
	.repartition($"data_source", $"ck_dt", $"mktng_dt")
	.write
	.partitionBy("data_source", "ck_dt", "mktng_dt")
	.mode("append")
	.save("hdfs://apollo-phx-nn-ha/user/hive/warehouse/mktng.db/marketing_txns_correlation_d30_test_batch")
