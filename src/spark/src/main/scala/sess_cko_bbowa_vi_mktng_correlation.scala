...
object SessCkoBbowaVI_MktngCorrelationApp {
 ...
 def main(args: Array[String]) {
   ...
   //FILTER FOR DATES
   // ========================
   val sess_df   = sess_xid_df.filter(s"dt between '$sess_start_dt' and '$sess_end_dt'").sample(false, 0.01)
   sess_df.cache()
   //
   //TAKE ONLY CLICKS
   ...
   .save("hdfs://apollo-phx-nn-ha/user/hive/warehouse/mktng.db/sess_cko_bbowa_vi_mktng_corr")
  )
