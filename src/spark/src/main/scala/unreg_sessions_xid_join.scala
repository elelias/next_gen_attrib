...
object unreg_sessions_with_xid {
 ...
 def main(args: Array[String]) {
   ...
   //============================
   spark.read.load("hdfs://apollo-phx-nn-ha/user/hive/warehouse/mktng.db/unregistered_users").createOrReplaceTempView("unreg_sess")
   ...
   coalesce(A.cobrand,B.cobrand, C.cobrand) as cobrand,
   coalesce(A.session_start_dt,B.session_start_dt, C.session_start_dt) as session_start_dt,
   coalesce(A.start_timestamp,B.start_timestamp, C.start_timestamp) as start_timestamp,
   coalesce(A.end_timestamp,B.end_timestamp, C.end_timestamp) as end_timestamp,
   coalesce(A.primary_app_id,B.primary_app_id, C.primary_app_id) as primary_app_id,
   ...
   coalesce(A.cust_traffic_source_level1,B.cust_traffic_source_level1, C.cust_traffic_source_level1) as cust_traffic_source_level1,
   coalesce(A.cust_traffic_source_level2,B.cust_traffic_source_level2, C.cust_traffic_source_level2) as cust_traffic_source_level2,
   coalesce(A.traffic_source_level3,B.traffic_source_level3, C.traffic_source_level3) as traffic_source_level3,
   coalesce(A.rotation_id,B.rotation_id, C.rotation_id) as rotation_id,
   coalesce(A.rvr_id,B.rvr_id, C.rvr_id) as rvr_id,
   coalesce(A.idfa,B.idfa, C.idfa) as idfa,
   coalesce(A.gadid,B.gadid, C.gadid) as gadid,
   coalesce(A.gdid,B.gdid, C.gdid) as gdid,
   coalesce(A.device_id,B.device_id, C.device_id) as device_id,
   coalesce(A.device_type,B.device_type, C.device_type) as device_type,
   ...
   coalesce(A.vi_cnt,B.vi_cnt, C.vi_cnt)         as vi_cnt,
   coalesce(A.incdata_id,B.incdata_id  )         as incdata_id,
   coalesce(A.join_strategy, B.join_strategy, 'nojoin') as join_strategy,
   coalesce(A.reg_dt, B.reg_dt, C.reg_dt)        as dt
   from            cguid_join A
   full outer join did_join   B
   on              A.guid         = B.guid
   and             A.session_skey = B.session_skey

   full outer join unreg_sess_indate  C
   on              A.guid         = C.guid
   ...
   val path="hdfs://apollo-phx-nn-ha/user/hive/warehouse/mktng.db/unregistered_sessions_xid_parquet"     
