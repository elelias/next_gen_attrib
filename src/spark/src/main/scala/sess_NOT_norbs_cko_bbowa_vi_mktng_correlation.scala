...
//
//
object Sess_NON_NORBS_CkoBbowaVI_MktngCorrelationApp {
 ...
 def main(args: Array[String]) {
   ...
   val sess_df   = sess_xid_df.filter(s"dt between '$sess_start_dt' and '$sess_end_dt' and has_first_purchase = 0").sample(false, 0.20)
   sess_df.cache()
   ...
   .save("hdfs://apollo-phx-nn-ha/user/hive/warehouse/mktng.db/sess_non_norbs_cko_bbowa_vi_mktng_corr")
  )
 }
}
