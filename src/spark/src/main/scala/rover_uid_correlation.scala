...
object CorrelationApp {
 ...
 def main(args: Array[String]) {
   ...
   "rover" as data_source,
   "event_dt" as mktng_dt,
   "create_dt" as ck_dt
   ...
   //
   join_df.write.save("hdfs://apollo-phx-nn-ha/user/hive/warehouse/mktng.db/correlation.parquet")
   //join_df.write.partitionBy("data_source", "mktng_dt", "ck_dt").save("hdfs://apollo-phx-nn-ha/user/hive/warehouse/mktng.db/correlation.parquet")
   //
   spark.stop()
  }
}
