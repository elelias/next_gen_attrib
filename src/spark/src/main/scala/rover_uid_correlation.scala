...
object CorrelationApp {
 ...
 def main(args: Array[String]) {
   ...
   val cko_dt = args(0)
   val data_source = args(1)
   val mktng_start_dt = args(2)
   val mktng_end_dt   = cko_dt
   ...
   val txns_path =  s"hdfs://apollo-phx-nn-ha/user/hive/warehouse/txns.db/txns_with_xid_parquet/dt=$cko_dt"
   ...
   merge_all_join_df.write.save("hdfs://apollo-phx-nn-ha/user/hive/warehouse/mktng.db/testing_correlations.parquet")
   ...
   def uid_join(){
     ...
     val query_path = ""
     val join_query = sc.wholeTextFiles(query_path).collect()(0)._2
     val join_df = sql(join_query)
     ...
     //=======================
     val mktng_non_null_uid_df = mktng_df.filter($"user_id" >0)
     mktng_non_null_uid_df.createOrReplaceTempView("mktng")

     //========================
     //JOIN THEM
     //========================
     val uid_join_df = sql(join_query)
   }
